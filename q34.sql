...
FROM
  ...
  WHERE tpcds.store_sales.ss_sold_date_sk = tpcds.date_dim.d_date_sk
    ...
    AND (tpcds.date_dim.d_dom BETWEEN 1 AND 3 OR tpcds.date_dim.d_dom BETWEEN 25 AND 28)
    ...
    THEN tpcds.household_demographics.hd_dep_count / tpcds.household_demographics.hd_vehicle_count
         ELSE NULL
         END) > 1.2
    AND tpcds.date_dim.d_year IN (1999, 1999 + 1, 1999 + 2)
    AND tpcds.store.s_county IN
